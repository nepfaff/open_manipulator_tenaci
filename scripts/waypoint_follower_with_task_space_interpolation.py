...
from open_manipulator_tenaci.trajectory_planning import (
    compute_task_space_trajectory_setpoints,
)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if current_waypoint == 0:
            ...
            # Convert into set points
            start = waypoints[current_waypoint - 1]
            waypoint = waypoints[current_waypoint]
            sample_period = 0.1
            set_points = compute_task_space_trajectory_setpoints(
                start, waypoint, 2, sample_period
            )

            # Publish set points to Gazebo topics
            for set_point in set_points:
                joint1_publisher.publish(set_point.joint_angles.joint1_angle)
                joint2_publisher.publish(set_point.joint_angles.joint2_angle)
                joint3_publisher.publish(set_point.joint_angles.joint3_angle)
                joint4_publisher.publish(set_point.joint_angles.joint4_angle)
